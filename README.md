...
Git을 사용하여 로컬에서 파일을 커밋하고 원격 저장소로 푸시하는 기본적인 명령어와 절차 안내
...
- **`git add .`**: 현재 디렉토리 및 하위 디렉토리의 모든 변경 사항을 스테이징 영역에 추가.
- **`git commit -m "메시지"`**: 스테이징된 파일을 커밋하며 메시지를 작성.

### 유용한 팁:

- **스테이징된 파일 및 변경 파일 상태 확인**:
  ```bash
  git status
  ```
- **특정 파일/경로로만 추가**:
...
- **`.gitignore` 파일**:
  - `.gitignore`에 정의된 패턴에 일치하는 파일은 `git add .` 명령으로 추가되지 않습니다.

---

## 3. 원하지 않는 파일 스테이징에서 제거
...
- **`restore`**: 스테이징 영역에서 파일을 제거하고 작업 디렉토리로 되돌림.
- **`reset`**: 스테이징 영역에서 파일을 제거.
...
- **`-u` 옵션**: 로컬 브랜치와 원격 브랜치를 연결(추적)합니다.
- `origin`: 원격 저장소의 이름.
- `main`: 기본 브랜치 이름(저장소에 따라 다를 수 있음).
